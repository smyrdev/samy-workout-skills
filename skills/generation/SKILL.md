...
7. **Echo before writing** — the planned-versus-allocated table, the week itself, every warning,
...
8. **Write only under `profiles/<slug>/plans/`** — never to `profile.json`, never under
   `programs/`, never to another person's directory.
